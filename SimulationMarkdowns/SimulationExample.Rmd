...
Grouped_DF %>% 
  ...
  XXXX %>%
    gather('Partition', 'Value',  MeanCodist, MeanR2, MeanSpa, MeanEnv  ) %>%
    ggplot( aes( x = Sample, y = Value, col = Partition))+
    geom_point()+
...
# ggsave('Figures/TernaryPlot.png', width = 6, height = 6, dpi = 500 )
...
xxx %>% 
  ...
  geom_tile()+scale_fill_viridis_c()+
    theme_nothing()+coord_fixed()+scale_alpha(range = c(0.2, 1)) +
  ggtitle( 'Density Pre-change') -> plotA
...
xxx %>% 
  ...
  geom_tile()+scale_fill_viridis_c()+
    theme_nothing()+coord_fixed()+
  ggtitle( 'Density (End)') -> plotB

xxx %>% ggplot(aes(x=x,y =y,fill=E1,
             alpha = Kept))+geom_tile()+
  scale_fill_viridis_c(option = 'B')+coord_fixed()+ theme_nothing()+
  ggtitle('Envrionmental Variable 1 (Pre-change)') -> plotC

xxx %>% ggplot(aes(x=x,y =y,fill=E2,
             alpha = Kept))+geom_tile()+
  scale_alpha(range = c(0.2, 1))+coord_fixed()+ theme_nothing()+
  scale_fill_viridis_c(option = 'B')+ ggtitle('Envrionmental Variable 2') -> plotD


xxx %>% ggplot(aes(x=x,y =y,fill=R_Tot,
             alpha = Kept))+geom_tile()+coord_fixed()+
  scale_alpha(range = c(0.2, 1))+ theme_nothing()+
  scale_fill_viridis_c(option = 'A') + geom_tile()+ ggtitle('R (Pre_Change)') -> plotE

xxx %>% ggplot(aes(x=x,y =y,fill=R_Tot_change,
             alpha = Kept))+geom_tile()+coord_fixed()+
  scale_alpha(range = c(0.2, 1))+ theme_nothing()+
  scale_fill_viridis_c(option = 'A') + geom_tile()+ ggtitle('R (After change)') -> plotF
...
plot_grid(  plotC, plotD,
            ...
            labels = c( 'a) E1', 'b) E2',
                        ...
                        'e) Density t =0',
                        'f) Density t= 25'),
            label_fontface = 'plain', scale = 0.9)

#ggsave( 'Figures/SimulationExample.png', height = 10, width = 5, dpi = 500)
