...
This repeat was run on a desktop with a different parameterisation that has totally random interactions between species 

This needed two new files:

BuildMC_rand

and 

Network Initiation_Rand.R
...
InputParameters$comp_conn= 0.025 ## increase to compensate for lack of off diagonal
...
```{r eval = FALSE}
Outs<-list.files(path ='TrialRand_VP', full.names = TRUE)
AllRuns<-map_df( Outs,read_csv, show_col_types = FALSE)
write_csv(AllRuns, 'FullRandomSimulation.csv')
```

```{r}
InputParameters <- read_csv(paste0('../Parameters/' ,TrialName, '.csv' ))
AllRuns <- read_csv( 'FullRandomSimulation.csv')
AllRuns%>%
...
AllData_logg %>%
  ...
  rename(  'Dispersal (log delta)' = Disp , 'Competition (alpha)'  =comp_str ,
         'Growth (gamma)' = R_max,'Stochasticity (log sigma)' = EnvNoiseSigma ) %>%
